...
def main():
    ...
    data_vendas_final = pd.read_csv("Dados de venda Final.csv").drop("Unnamed: 0", axis = 1)

    receitaDevenda = receita_vendas(data_vendas_final) #Valor da receita
    devolucoesComissao = devolucoes_TK(data_vendas_final) #valor da comissição tk dos produtos devolvidos
    ...
    devolucoesCusto = devolucoes_Custo(data_vendas_final)
    comicaoTk = comissao_tk(data_vendas_final) #Valor da Taxa de Comissão do TK
